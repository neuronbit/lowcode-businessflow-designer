...
export const saveSchema = async (schema: any) => {
    // await setPackagesToLocalStorage(scenarioName);
    window.saveSchema(JSON.stringify(schema, null, 2));
...
export const loadSchema = async (file: string|null = 'unknown'): Promise<any> => {
    // @ts-ignore
    //let schema = window.callJava('{event:\'openFile\', file:'+ file+'}')
    return schema;
}
